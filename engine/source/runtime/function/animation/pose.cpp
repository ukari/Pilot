...
void AnimationPose::blend(const AnimationPose& pose)
{
    ...
    for (int i = 0; i < m_bone_poses.size(); i++)
    {
        ...
        if (sum_weight != 0)
        {
          ...
          m_weight.m_blend_weight[i] = (m_weight.m_blend_weight[i] * m_weight.m_blend_weight[i] + pose.m_weight.m_blend_weight[i] * pose.m_weight.m_blend_weight[i]) / sum_weight;
